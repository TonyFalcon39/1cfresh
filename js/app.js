        ...
        async function formSubmitAction(form, e) {
            ...
            if (0 === error) {
                ...
                if (ajax) {
                    ...
                    if (e.target.closest(".begin__form")) if (document.querySelector(".programs__items")) {
                        let programsBlock = document.querySelector(".programs__items");
                        let programs = programsBlock.querySelectorAll(".item-programs");
                        let progChosen = document.querySelector(".programs__chosen");
                        if (programsBlock.querySelector("._chosen")) progChosen.innerHTML = ""; else progChosen.innerHTML = "Все программы";
                        for (let i = 0; i < programs.length; i++) {
                            let program = programs[i];
                            if (program.classList.contains("_chosen")) {
                                let titleProg = program.querySelector(".item-programs__title").innerHTML;
                                progChosen.innerHTML += titleProg + ";\n";
                            }
                        }
                        formData.append("programs", progChosen.innerHTML);
                    }
                    ...
                    if (response.ok) {
                        ...
                        if (form.classList.contains("callback-form")) {
                            form.querySelector(".popup-order__response").classList.add("_showed");
                            setTimeout((() => {
                                form.querySelector(".popup-order__response").classList.remove("_showed");
                            }), 2e3);
                        }
                        formSent(form);
                    } else {
 ...
 */
    ...
    da.init();
    let itemPrograms = document.querySelectorAll(".item-programs");
    let progBtn = document.querySelector(".prog__btn");
    document.querySelector(".programs__chosen");
    if (document.querySelector(".item-programs")) for (let i = 0; i < itemPrograms.length; i++) {
        let itemProgram = itemPrograms[i];
        itemProgram.querySelector(".item-programs__title");
        itemProgram.addEventListener("click", (function() {
            itemProgram.classList.toggle("_chosen");
            if (document.querySelector(".programs").querySelector("._chosen")) progBtn.innerHTML = "Подключить"; else progBtn.innerHTML = "Подключить все";
        }));
    }
    window["FLS"] = false;
    isWebp();
